...
export default function Finance() {
  ...
  const handleAdd = () => {
    ...
    setTxAmount(''); setTxDescription(''); setTxDate(new Date().toISOString().split('T')[0]); setShowModal(false);
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <span style={{ fontWeight: 600, fontSize: '0.95rem', color: 'var(--text-primary)' }}>{monthLabel(viewMonth)}</span>
        <button className="btn-ghost" onClick={() => changeMonth(1)}>
      ...
      <div className="finance-summary">
        <div className="finance-card">
          <div className="amount" style={{ color: 'var(--success)' }}>{formatAmount(monthData.income)}</div>
        ...
        <div className="finance-card">
          <div className="amount" style={{ color: monthData.balance >= 0 ? 'var(--success)' : 'var(--accent)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
        <span style={{ fontSize: '0.8rem', color: 'var(--text-tertiary)' }}>Moneda:</span>
      ...
      <div className="section-title">Transacciones</div>
            ...
            return (
              <div key={tx.id} className="transaction-item">
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 600, fontSize: '0.85rem' }}>{tx.description || catInfo.label}</div>
                  <div style={{ fontSize: '0.72rem', color: 'var(--text-disabled)' }}>{tx.date} · {catInfo.label}</div>
                </div>
                <div style={{
                  fontWeight: 700, fontSize: '0.88rem',
                  color: tx.type === 'income' ? 'var(--success)' : 'var(--accent)',
                  marginRight: 8, whiteSpace: 'nowrap',
                }}>
                  {tx.type === 'income' ? '+' : '-'}{formatAmount(tx.amount)}
                </div>
                <button className="btn-ghost" onClick={() => { if (window.confirm('Eliminar?')) deleteTransaction(tx.id); }}>
      ...
      <button className="btn-primary" onClick={() => setShowModal(true)}>Agregar transaccion</button>
        ...
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal-content">
            ...
            <div className="tab-group" style={{ marginBottom: 16 }}>
              <button className={`tab-btn ${txType === 'expense' ? 'active' : ''}`}
                onClick={() => { setTxType('expense'); setTxCategory('comida'); }}>Gasto</button>
              <button className={`tab-btn ${txType === 'income' ? 'active' : ''}`}
                onClick={() => { setTxType('income'); setTxCategory('salario'); }}>Ingreso</button>
            </div>
