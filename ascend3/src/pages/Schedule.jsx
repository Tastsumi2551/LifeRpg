...
export default function Schedule() {
  ...
  const handleAddSubject = () => {
    ...
    addSubject({ name: subName.trim(), professor: subProfessor.trim(), color: subColor, room: subRoom.trim() });
    setSubName(''); setSubProfessor(''); setSubRoom('');
  ...
  const handleAddExam = () => {
    ...
    addExam({ name: examName.trim(), subjectId: examSubjectId, date: examDate });
      ...
      {tab === 'horario' && (
        <>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.82rem', marginBottom: 14 }}>
            Toca una celda vacia para agregar. Toca un bloque para eliminarlo.
          </p>
                      ...
                      return (
                        <td key={dayKey}>
                          <div
                            ...
                            style={data ? { background: `${cat.color}cc` } : {}}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 18 }}>
              {subjects.map((sub) => (
                <div key={sub.id} className="subject-card">
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 700, fontSize: '0.95rem', marginBottom: 2 }}>{sub.name}</div>
                    {sub.professor && <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>{sub.professor}</div>}
                    {sub.room && <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>{sub.room}</div>}
                  </div>
                  <button className="btn-ghost" onClick={() => { if (window.confirm('Eliminar materia y sus examenes?')) deleteSubject(sub.id); }}>
                    🗑️
                  </button>
                </div>
              ))}
      ...
      {tab === 'examenes' && (
        <>
          {sortedExams.length === 0 ? (
            <div className="empty-state">
              <div className="empty-icon">📝</div>
              <p>No tienes examenes programados.</p>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      padding: '14px 16px', opacity: isPast ? 0.5 : 1,
                      borderLeft: `3px solid ${sub?.color || 'var(--accent)'}`,
                    }}
                  >
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 700, fontSize: '0.9rem', marginBottom: 2 }}>{exam.name}</div>
                    ...
                    <button className="btn-ghost" onClick={() => { if (window.confirm('Eliminar examen?')) deleteExam(exam.id); }}>
                      🗑️
                    </button>
                  </div>
                );
          ...
          <button className="btn-primary" onClick={() => { setExamSubjectId(subjects[0]?.id || ''); setShowExamModal(true); }}>
            + Agregar Examen
          </button>
        </>
      )}
        ...
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setShowBlockModal(false)}>
          <div className="modal-content">
            ...
            <div className="form-group">
              ...
              <input className="form-input" value={blockName} onChange={(e) => setBlockName(e.target.value)}
                placeholder="Ej: Matematicas, Gym..." autoFocus onKeyDown={(e) => e.key === 'Enter' && handleAddBlock()} />
            </div>
            <div className="form-group">
              <label className="form-label">Categoria</label>
              <select className="form-input" value={blockCategory} onChange={(e) => setBlockCategory(e.target.value)}>
                {SCHEDULE_CATEGORIES.map((c) => <option key={c.value} value={c.value}>{c.icon} {c.label}</option>)}
              </select>
            </div>
      ...
      {showSubjectModal && (
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setShowSubjectModal(false)}>
          <div className="modal-content">
            ...
            <div className="form-group">
              <label className="form-label">Nombre</label>
              <input className="form-input" value={subName} onChange={(e) => setSubName(e.target.value)}
                placeholder="Ej: Calculo II" autoFocus />
            </div>
            <div className="form-group">
              <label className="form-label">Profesor (opcional)</label>
              <input className="form-input" value={subProfessor} onChange={(e) => setSubProfessor(e.target.value)}
                placeholder="Ej: Prof. Garcia" />
            </div>
            <div className="form-group">
              <label className="form-label">Aula (opcional)</label>
              <input className="form-input" value={subRoom} onChange={(e) => setSubRoom(e.target.value)}
      ...
      {showExamModal && (
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setShowExamModal(false)}>
          <div className="modal-content">
            ...
            <div className="form-group">
              <label className="form-label">Nombre</label>
              <input className="form-input" value={examName} onChange={(e) => setExamName(e.target.value)}
                placeholder="Ej: Parcial 1" autoFocus />
            </div>
            {subjects.length > 0 && (
              <div className="form-group">
